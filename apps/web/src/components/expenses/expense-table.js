...
function PaidByCellRenderer(props) {
    if (!props.data)
        return null;
...
function BucketCellRenderer(props) {
    if (!props.data)
        return null;
...
function ReceiptCellRenderer(props) {
    if (!props.data)
        return null;
...
// --- Main component ---
export function ExpenseTable({ expenses, members, buckets, 
// eslint-disable-next-line @typescript-eslint/no-unused-vars
grantMode: _grantMode, onOpenModal, onOpenReceipts, undoStack, onSelectionChange, }) {
    ...
    const PaidByRenderer = useMemo(() => {
        return function PaidByRendererInner(props) {
            return <PaidByCellRenderer {...props} members={members}/>;
        };
    }, [members]);
    const BucketRenderer = useMemo(() => {
        return function BucketRendererInner(props) {
    ...
    const paidByOptions = useMemo(() => ["", ...members.map((m) => m.userId)], [members]);
    const bucketOptions = useMemo(() => ["", ...buckets.map((b) => b.id)], [buckets]);
    // --- Column definitions ---
    const columnDefs = useMemo(() => {
        const paidByValueMap = new Map();
        paidByValueMap.set("", "Unassigned");
        for (const m of members)
            paidByValueMap.set(m.userId, m.userName);
        const bucketValueMap = new Map();
        bucketValueMap.set("", "No bucket");
        for (const b of buckets)
            bucketValueMap.set(b.id, b.name);
        return [
            {
                headerName: "",
                width: 48,
            ...
            },
            {
                ...
                valueFormatter: (params) => params.value != null ? (params.value / 100).toFixed(2) : "",
                valueParser: (params) => {
                    const num = parseFloat(String(params.newValue).replace(/[^0-9.]/g, ""));
            ...
            },
            {
                ...
                filter: "agTextColumnFilter",
                filterValueGetter: (params) => statusLabel(params.data?.status ?? ""),
            },
            {
                headerName: "Paid By",
                field: "paidById",
                ...
                valueFormatter: (params) => paidByValueMap.get(params.value ?? "") ?? "Unassigned",
                filter: "agTextColumnFilter",
                filterValueGetter: (params) => paidByValueMap.get(params.data?.paidById ?? "") ?? "Unassigned",
            },
            {
                headerName: "Bucket",
                field: "bucketId",
                ...
                valueFormatter: (params) => bucketValueMap.get(params.value ?? "") ?? "No bucket",
                filter: "agTextColumnFilter",
                filterValueGetter: (params) => bucketValueMap.get(params.data?.bucketId ?? "") ?? "No bucket",
            },
            {
                headerName: "Notes",
                field: "notes",
    ...
    const onCellEdit = useCallback((expenseId, field, value) => {
        const expense = expenses.find((e) => e.id === expenseId);
        if (expense) {
            undoStack.push(expense, field, expense[field], value);
        }
        updateExpense.mutate({ id: expenseId, [field]: value });
    ...
    const handleBulkStatusChange = useCallback((status) => {
        ...
        const batch = selectedIds
            ...
            .filter(Boolean);
        undoStack.pushBatch(batch.map((e) => ({ expense: e, field: "status", oldValue: e.status })));
    ...
    useEffect(() => {
        return subscribeLive((ids) => {
            if (onSelectionChangeRef.current) {
                onSelectionChangeRef.current(expensesRef.current.filter((e) => ids.has(e.id)));
            }
        });
    }, [subscribeLive]);
    ...
    return (<div className="space-y-2">
      ...
      <div className="flex items-center gap-2 rounded-lg bg-muted px-4 py-2 text-sm">
        ...
        <div className="ml-2 flex items-center gap-1">
          <Button variant="secondary" size="sm" className="h-7" disabled={selectedCount === 0} onClick={() => handleBulkStatusChange("paid")}>
            <ArrowRightLeft className="mr-1 size-3.5"/>
            Mark Paid
          </Button>
          <Button variant="secondary" size="sm" className="h-7" disabled={selectedCount === 0} onClick={() => handleBulkStatusChange("approved")}>
            Mark Approved
          </Button>
          <Button variant="secondary" size="sm" className="h-7" disabled={selectedCount === 0} onClick={() => handleBulkStatusChange("outstanding")}>
            Mark Outstanding
          </Button>
          <Button variant="destructive" size="sm" className="h-7" disabled={selectedCount === 0} onClick={handleBulkDelete}>
            <Trash2 className="mr-1 size-3.5"/>
            Delete
          </Button>
        </div>
        <Button variant="ghost" size="sm" className="ml-auto h-7" disabled={selectedCount === 0} onClick={clearSelection}>
          <X className="mr-1 size-3.5"/>
          Deselect
        </Button>
      </div>

      <div ref={containerRef} {...getContainerProps()} style={{
            ...getContainerProps().style,
            height: "calc(100vh - 280px)",
            minHeight: 300,
        }}>
        <AgGridReact ref={gridRef} theme={agTheme} rowData={expenses} columnDefs={columnDefs} defaultColDef={defaultColDef} getRowId={(params) => params.data.id} rowSelection="multiple" suppressRowClickSelection={false} onGridReady={onGridReady} onCellValueChanged={onCellValueChanged} onSelectionChanged={onSelectionChanged} onColumnResized={saveColumnState} onColumnMoved={saveColumnState} onSortChanged={saveColumnState} onCellContextMenu={onCellContextMenu} stopEditingWhenCellsLoseFocus={true} suppressContextMenu={true} animateRows={false} noRowsOverlayComponent={() => (<div className="py-8 text-muted-foreground">
              No expenses yet. Click "Add Expense" to get started.
            </div>)}/>

        {/* Marquee selection rectangle */}
        <div ref={marqueeRef} className="pointer-events-none absolute border border-primary/60 bg-primary/10" style={{ zIndex: 20, display: "none" }}/>
      </div>

      {/* Context menu portal */}
      {contextMenuExpense && contextMenuPos && (<ContextMenuPortal expense={contextMenuExpense} members={members} position={contextMenuPos} onOpenModal={onOpenModal} onOpenReceipts={onOpenReceipts} onCellEdit={onCellEdit} onDelete={() => deleteExpense.mutate(contextMenuExpense.id)} onClose={() => {
                setContextMenuExpense(null);
                setContextMenuPos(null);
            }}/>)}
...
function ContextMenuPortal({ expense, members, position, onOpenModal, onOpenReceipts, onCellEdit, onDelete, onClose, }) {
    ...
    return (<div ref={menuRef} className="fixed z-50 min-w-[200px] rounded-md border bg-popover p-1 text-popover-foreground shadow-md" style={{ left: position.x, top: position.y }}>
      <button className="flex w-full items-center rounded-sm px-2 py-1.5 text-sm hover:bg-accent" onClick={() => {
            onOpenModal(expense);
            onClose();
        }}>
        Edit
      ...
      <button className="flex w-full items-center rounded-sm px-2 py-1.5 text-sm hover:bg-accent" onClick={() => {
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const { id, createdAt, updatedAt, receiptCount, createdById, ...rest } = expense;
            onOpenModal(rest);
            onClose();
        }}>
        Duplicate
      ...
      {statuses.map((s) => (<button key={s.value} className={`flex w-full items-center rounded-sm px-2 py-1.5 text-sm hover:bg-accent ${expense.status === s.value ? "font-semibold" : ""}`} onClick={() => {
      ...
      <button className={`flex w-full items-center rounded-sm px-2 py-1.5 text-sm hover:bg-accent ${!expense.paidById ? "font-semibold" : ""}`} onClick={() => {
            onCellEdit(expense.id, "paidById", null);
            onClose();
        }}>
        Unassigned
      </button>
      {members.map((m) => (<button key={m.userId} className={`flex w-full items-center rounded-sm px-2 py-1.5 text-sm hover:bg-accent ${expense.paidById === m.userId ? "font-semibold" : ""}`} onClick={() => {
      ...
      <div className="my-1 h-px bg-border"/>
      <button className="flex w-full items-center rounded-sm px-2 py-1.5 text-sm text-destructive hover:bg-destructive/10" onClick={() => {
            onDelete();
            onClose();
        }}>
        Delete
