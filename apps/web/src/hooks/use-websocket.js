...
export function useExpenseWebSocket() {
    ...
    const connect = useCallback(async () => {
        ...
        let token;
        try {
            const res = await api("/auth/ws-token", {
                method: "POST",
            });
            token = res.token;
        }
        ...
        const ws = new WebSocket(`${WS_BASE}/api/ws?token=${encodeURIComponent(token)}&eventId=${encodeURIComponent(currentEvent.id)}`);
        ws.onmessage = (event) => {
            try {
                const msg = JSON.parse(event.data);
                if (msg.type === "expense_created" ||
